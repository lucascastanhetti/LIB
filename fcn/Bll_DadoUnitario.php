...
class Bll_DadoUnitario {
    ...
    public function getDados($ls_dadosExibicao, $intervalo, $sensores) {
        ...
        $intervalo = $this->findIntervalo($ls_dadosExibicao);
	if ($intervalo == 0) {
        ...
        foreach ($ls_dadosExibicao as $dados) {
            ...
            if ((@$lastData == null) || (@$lastData != $dados->getColetahorario())) {
                ...
                if (@$lastData != null) {
                    ...
                    if (($timeDif == $intervalo) || ($lastEstacao != $dados->getEstacaoid())) {
                        ...
                        //correção para não subcrever o primeiro dado
            ...
            }elseif($dados->getSensornome() == 'Nivel2') {
                $dadoUnitario->setNivel2($dados->getValor());
            ...
            }elseif ($dados->getSensornome() == 'VazaoDefluente') {
                $dadoUnitario->setVazaoDefluente($dados->getValor());
    ...
    public function findIntervalo($ls_dadosExibicao) {
        $hashdado[] = array();
        $geral = new Geral();
	...
	foreach ($ls_dadosExibicao as $dados) {
        ...
        $intervalo = 0;
        $maxOcorrencias = 0;
        foreach (array_keys($hashdado) as $d) {
            if ($d > 0) {
                if ($hashdado[$d] > $maxOcorrencias) {
                    $maxOcorrencias = $hashdado[$d];
                    $intervalo = $d;
                }
            }
        }

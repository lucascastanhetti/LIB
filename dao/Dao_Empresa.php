...
include(dirname(__FILE__) . '/../model/VO_Empresa.php');
...
class Dao_Empresa extends Dao_ConnectionFactory {
    ...
    public function insert(VO_Empresa $vo) {
        ...
        $SQL = "INSERT INTO empresa(abreviatura,cnpj,contato,logo,nome,tel)";
        $SQL .= " VALUES(?,?,?,?,?,?,?)";
    ...
    public function update(VO_Empresa $vo) {
        $SQL = "UPDATE empresa SET abreviatura = ?,cnpj = ?,contato = ?,logo = ?,nome = ?,tel = ?";
        $SQL .= " WHERE id = ?";
        $query = $this->conn->prepare($SQL);
        $query->bindValue(1, $vo->getAbreviatura(), PDO::PARAM_STR);
        $query->bindValue(2, $vo->getCnpj(), PDO::PARAM_STR);
    ...
    public function getAll() {
        ...
        while ($r = $query->fetch()) {
            $vo = new VO_Empresa();
            $vo->setId($r["id"]);
            $vo->setAbreviatura($r["abreviatura"]);
            $vo->setCnpj($r["cnpj"]);
            $vo->setContato($r["contato"]);
            $vo->setLogo($r["logo"]);
            $vo->setNome($r["nome"]);
            $vo->setTel($r["tel"]);
            $vo->sethasBalancoHidrico($r["hasbalancohidrico"]);
            $vo->sethasPrevisaoGeracao($r["hasprevisaogeracao"]);
            $vo->setHasKirpich($r["haskirpich"]);
            $vo->setHasCurvaPermanencia($r["hascurvapermanencia"]);
